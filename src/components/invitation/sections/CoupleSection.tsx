...
export function CoupleSection({ tenant, themeConfig }: Props) {
  ...
  const groomCard = (
    ...
    >
      ...
      {tenant.groomInstagram && (
        <a
          href={`https://instagram.com/${tenant.groomInstagram.replace("@", "")}`}
          target="_blank"
          rel="noopener noreferrer"
          className="mt-3 inline-flex items-center gap-1.5 text-sm transition-opacity hover:opacity-70"
        ...
        >
          ...
          <span>{tenant.groomInstagram.startsWith("@") ? tenant.groomInstagram : `@${tenant.groomInstagram}`}</span>
        </a>
      )}
    </motion.div>
  );
  ...
  const brideCard = (
    ...
    >
      ...
      {tenant.brideInstagram && (
        <a
          href={`https://instagram.com/${tenant.brideInstagram.replace("@", "")}`}
          target="_blank"
          rel="noopener noreferrer"
          className="mt-3 inline-flex items-center gap-1.5 text-sm transition-opacity hover:opacity-70"
          style={{ color: themeConfig.accentColor }}
        >
          ...
          <span>{tenant.brideInstagram.startsWith("@") ? tenant.brideInstagram : `@${tenant.brideInstagram}`}</span>
        </a>
      )}
    </motion.div>
  );
      ...
      <div className="max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="text-center mb-16 scroll-mt-6">
          <p className="font-script text-2xl mb-2" style={{ color: themeConfig.primaryColor }}>
            {isJawaHijau ? "Assalamu'alaikum" : isJawa ? "Pandhan Arum" : isIslam ? content.openingGreeting : "In The Name of Love"}
          </p>
          <h2 className="section-title font-display" style={{ color: themeConfig.textColor }}>
            {isJawaHijau ? "Pasangan Pengantin" : isJawa ? "Kekalih Panganten" : "Mempelai"}
          </h2>
        </motion.div>
